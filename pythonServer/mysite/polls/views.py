...
class DetailView(generic.DetailView):
    model = Question
        ...
        #render(request,'polls/index.html', {'predict_data': model2})
...
def viewModelData(request, modelData):
    ...
    #모델링후 아래에서 모델을  사용합니다
    #print("2022-07-11 의 경우엔 : ",)
    print("2022-07-12 의 경우엔 ghrdls: ",line_fitter.predict([[24]]))
    print("2022-07-13 의 경우엔 : ",line_fitter.predict([[39]]))
    #예측결과를 출력해줍니다
    #return render(request, 'polls/detail.html', {'predict_data':line_fitter.predict([[24]])})
    #model = Question

    return HttpResponse("예측된 데이터 : " + str(int(line_fitter.predict([[modelData]]))))
